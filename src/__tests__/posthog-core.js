...
describe('capture()', () => {
  ...
  given('subject', () => () => given.lib.capture(given.eventName, given.eventProperties, given.options, given.callback))
...
describe('bootstrapping feature flags', () => {
  ...
  it('does nothing when empty', () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('getFeatureFlag for key "multivariant" failed'))
    expect(given.lib.getFeatureFlag('disabled')).toBe(undefined)
    expect(given.lib.getFeatureFlag('undef')).toBe(undefined)
...
describe('_loaded()', () => {
  ...
  describe('capturing pageviews', () => {
    ...
    it('captures pageview if enabled', () => {
      ...
      expect(given.overrides.capture).toHaveBeenCalledWith('$pageview', { title: 'test' }, { send_instantly: true })
    })
  })
