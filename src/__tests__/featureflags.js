...
describe('featureflags', () => {
  ...
  describe('earlyAccessFeatures', () => {
    ...
    it('reloading flags after update enrollment should send properties', () => {
      ...
      expect(JSON.parse(Buffer.from(given.instance._send_request.mock.calls[0][1].data, 'base64').toString())).toEqual({
        token: 'random fake token',
        distinct_id: 'blah id',
        person_properties: {
          '$feature_enrollment/x-flag': true,
  ...
  describe('reloadFeatureFlags', () => {
    ...
    it('on providing anonDistinctId and calling reload multiple times', () => {
      ...
      expect(JSON.parse(Buffer.from(given.instance._send_request.mock.calls[1][1].data, 'base64').toString())).toEqual({
        token: 'random fake token',
        distinct_id: 'blah id',
        // $anon_distinct_id: "rando_id"
      })
      ...
      // check the request didn't send $anon_distinct_id the second time around
      expect(JSON.parse(Buffer.from(given.instance._send_request.mock.calls[2][1].data, 'base64').toString())).toEqual({
        token: 'random fake token',
        distinct_id: 'blah id',
        // $anon_distinct_id: "rando_id"
      })
