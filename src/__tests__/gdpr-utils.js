    ...
    forPersistenceTypes(function (persistenceType) {
      ...
      it(`should allow use of a custom "persistence prefix" string`, () => {
        TOKENS.forEach((token) => {
          gdpr.optIn(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })
          expect(gdpr.hasOptedIn(token, { persistenceType })).toBe(false)
          expect(gdpr.hasOptedIn(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })).toBe(true)
          gdpr.optOut(token)
          expect(gdpr.hasOptedIn(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })).toBe(true)
          gdpr.optOut(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })
          expect(gdpr.hasOptedIn(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })).toBe(false)
        })
      })
    })
  })
    ...
    forPersistenceTypes(function (persistenceType) {
      ...
      it(`should allow use of a custom "persistence prefix" string`, () => {
        TOKENS.forEach((token) => {
          gdpr.optOut(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })
          expect(gdpr.hasOptedOut(token, { persistenceType })).toBe(false)
          expect(gdpr.hasOptedOut(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })).toBe(true)
          gdpr.optIn(token, { persistenceType })
          expect(gdpr.hasOptedOut(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })).toBe(true)
          gdpr.optIn(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })
          expect(gdpr.hasOptedOut(token, { persistencePrefix: CUSTOM_PERSISTENCE_PREFIX, persistenceType })).toBe(false)
        })
      })
    })
  })
